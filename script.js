(function () {
  ...
  const fontSizeKey = "kang-taemin-font-size";
  const readerStateKey = "kang-taemin-reader-open";
  const readerScrollKey = "kang-taemin-reader-scroll";
  ...
  function updateReadingState() {
    ...
    const topButtonVisible = gate.hidden
  ...
  mapZoomIn.addEventListener("click", function () {
    if (mapZoomIndex === mapZoomLevels.length - 1) return;
    mapZoomIndex += 1;
    applyMapZoom(true);
  });
